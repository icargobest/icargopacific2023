...
@extends('layouts.app') @include('partials.navigationCompany',['order' =>
"nav-selected"])
...
<div class="container mw-100">
    <div class="bg-white shadow" style="max-width: 100%">
        <div
            class="waybill-head employee-header-container"
        ...
        <section class="mb-5 px-5 my-3 overflow-auto">
            <table class="table table-striped table-hover table-borderless hover" id="companyorder">
                ...
                <tbody>
                    ...
                    $ship->company_bid == null && $ship->status == 'Pending'))
                    @if(Auth::user()->type == 'company' && $ship->company_bid ==
                    null && $ship->status == 'Pending')
                    <tr>
                        <td>{{$ship->id}}</td>
                        <td style="width: 70px; height: 70px">
                            <img
                                src="{{asset($ship->photo)}}"
                                class="card shadow-0 w-25 h-100"
                                style="min-width: 70px; object-fit: cover"
                                alt=""
                            />
                        </td>
                        <td>
                        ...
                        <td>
                            <a
                                class="cardItem"
                                href="{{route('viewOrder_Company',$ship->id)}}"
                            >
                                <button
                                    type="button"
